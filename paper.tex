...
We implemented our new clause learning schemes in the
\text{\MapleBase} \cite{}, the winner of SAT Race 2015 application
track. We then evaluated our schemes on the full set of benchmarks
from SAT RACE 2019 main track which contains 400 instances. We ran our
experiments on \fb{insert GHz of CPU}, allowing \fb{insert time bound}
seconds per instance and a maximum of \fb{insert RAM bound}.

\fb{Delete figure 2 and use only figure 8 with all of the
  solvers. First briefly comment on the performance and make a brief
  observation about clause sizes.
...
$\MapleIUIPPure$ and $\MapleIUIMin$, we also captures the $\allUip$
...
green) is consistently smaller than $\allUipPure$ clauses.
...
$\allUipMin$ outperformed $\allUipPure$ in clause size.  This results agrees with our observation in Fig.~\ref{fig:t2}: $\allUipMin$ attempted $\allUip$ learning more frequently, and it is more likely to succeed.  
...
$\allUipMin$ and $\allUipPure$ are significantly smaller than 1-UIP
...
\hline
\nf{$\allUipPure$} &\textbf{228} (135, \nf{93}) & 4867.37 & 49.88 \\
\hline
$\allUipMin$ & 226 (135, 91) & 4890.67 & 45.2 \\ 
\hline
$\allUipAct$ & 226 (135, 91)  & \textbf{4866.94} & 47.7 \\
\hline
$\allUipIn$ & 225 (\textbf{138}, 87) & 4958.49 & 52..12 \\
\hline
$\allUipEx$ & 223 (134, 89) & 5015.23 & \textbf{43.2} \\
\hline
\end{tabular}
\end{center}
...
\subsection{$\allUip$ on Modern SAT solvers}
To validate $\allUip$ as a generalizable learning scheme on modern SAT
...
$\allUip$ outperformed 1-UIP learning for the SAT 2017 race winner,
$\MapleSeven$. More specifically, $\allUipPure$, $\allUipMin$,
$\allUipAct$ and $\allUipIn$ solved 12, 8, 5 and 2 more instances,
respectively, whiling producing smaller clauses. $\allUipPure$ solved
more UNSAT and SAT instances while other configurations improved on
solving SAT instances.  The improvement of $\allUip$ is more significant
on $\MapleSeven$ than on $\MapleBase$ for both solved instances and
clause size reduction. This may suggests that $\allUip$ and the recent
...
We observed significant improvement of $\allUip$ for the SAT 2018
winner, $\MapleEightShort$. Three out of four $\allUip$ configurations
...
configurations. The results indicates that the improvement of $\allUip$
...
$\allUip$ learning schemes showed significant improvement for
...
multiple times. $\allUip$ and DL didn't synergies well possibly because
i-UIP clauses are less likely to be duplicated. We observed that
$\allUipMin$ ,in average, added 12\% less duplicated clauses into the
...
We observed significant improvement of $\allUip$ for $\expSATShort$,
three out of four configurations of $\allUip$ significantly outperformed

...
\subsection{Greedy Active Clause Selection}
Even though $\IUIP$ learning can reduce the size of the learnt clause $\iUIPClause$, but it may introduce inactive literals into $\iUIPClause$.  Inactive literals prevents the clause from being asserted to force literal implication. Therefore, a practical clause learning scheme should consider both size and variable activity. We propose an optional extension $\IUIPGreedy$ to filter out inactive $\oneUIPClause$.  
...
\Require  $\oneUIPClause$ is a valid 1-UIP clause
\Require  $\assertionTrail$ is a valid assertion trail
\Procedure{$\IUIPGreedy$}{$\oneUIPClause,  \assertionTrail$} 
   \State $\iUIPClause \gets \IUIP(\oneUIPClause, \assertionTrail)$
...
\subsection{Adjust Variable Activity}
